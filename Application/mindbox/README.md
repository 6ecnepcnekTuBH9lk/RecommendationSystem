...
## Источник сохранённого набора и очередь справочников

Summary показывает API, «Вручную» или «Ручная + API» между датой обновления
и периодом. Для Actions/Orders source_kinds выводится из прежнего continuous_range:
учитываются только части выбранного покрытия. API-части, перекрытые ручным
интервалом, не дают ложного mixed-источника. Правила выбора training coverage
не изменены. CustomerMerges использует source_kind единственного canonical source.

В metadata summary базы customers.sqlite добавлен отсортированный source_kinds.
import_full начинает новый набор с ["MANUAL"]; apply_month объединяет прежний
набор с ["API"] в той же SQLite-транзакции, что и profiles/count/receipts.
Повторные API-обновления сохраняют mixed, следующий manual snapshot сбрасывает
происхождение. Старое поле source_kind сохранено для совместимости; если нового
набора нет, чтение использует старое поле без перезаписи базы. Исходные JSON
и metadata отдельных customer records не изменяются.

В блоке справочников три независимых селектора CSV. Общая кнопка запускает
только выбранные типы в порядке REFERENCE_TYPES через прежний CLI, строго по
одному QProcess. Успех вызывает apply_reference_result и обновляет UI. Ошибка
отмечается у конкретного типа и в журнале, очередь продолжает работу; общий
результат FAILED при любой ошибке. Успешные независимые публикации не откатываются.
Отмена останавливает оставшуюся очередь. Статусная строка находится под общей кнопкой.
